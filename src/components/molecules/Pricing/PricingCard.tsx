...
export const PricingCard = ({ data, maxPrice }: PricingCardProps) => {
  ...
  const [allowedWines, setAllowedWines] = useState<string>("");

  useEffect(() => {
    if (data.price === 0) {
      setAllowedWines("2 Free QR Codes");
      setPrice("Free");
    } else if (data.price > 0 && data.price < maxPrice) {
      setAllowedWines(data.qrCodes + " QR Codes");
      setPrice("$" + data.price.toLocaleString("en-US"));
    } else if (data.price === maxPrice) {
      setAllowedWines("Unlimited QR Codes");
      setPrice("$" + data.price.toLocaleString("en-US"));
    }
  }, [data]);
  return (
      ...
      >
        ...
        >
          ...
          {/* <ul className="space-y-[12px]">
            ...
            >
              <div className="min-w-[8px] min-h-[8px] max-w-[8px] max-h-[8px] mt-[8px] rounded-full bg-primary-light" />
              <li className="text-on-surface">{allowedWines}</li>
            </Container>
          </ul> */}
